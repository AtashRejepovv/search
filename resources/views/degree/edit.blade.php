...
@section('content')
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-7 col-md-5 offset-md-3">
            <form role="form" action="{{route('rank.update',['rank_id'=>$rank->id])}}" method="POST">
                {{ csrf_field() }}
                <div class="form-group">
                    <input type="text" name="name" id="name" class="form-control input-lg" value="{{$rank->name}}" placeholder="Familiýasy" tabindex="1">
                </div>
                <hr class="colorgraph">
